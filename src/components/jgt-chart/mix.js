...
export default {
  ...
  computed: {
    ...
    LineColorRgba() {
      ...
      mainColorRgbaLists.darkTheme = info.darkColor
        ? this.util.hexToRgba(info.darkColor, 0.1)
        : this.util.hexToRgba(this.constant.mainColorLists[name], 0.1)
      mainColorRgbaLists.lightTheme = info.lightColor
        ? this.util.hexToRgba(info.lightColor, 0.1)
        : this.util.hexToRgba(this.constant.mainColorLists[name], 0.1)
      ...
      try {
        this.$globalchar.on('legendselectchanged', function (params) {
          ...
          for (let seriesName in selectedLegend) {
            if (selectedLegend.hasOwnProperty(seriesName) && selectedLegend[seriesName]) {
              count++
            }
          }
